        ...
        <el-table-column label="操作" width="300">
          <template slot-scope="scope">
            <div v-if="scope.row.type=='file'">
              ...
              <el-button type="text" size="small"
                         @click="openDelete(scope.row.data.fileId,scope.row.type)">删除
              </el-button>
              <el-button type="text" size="small"
            ...
            <div v-else-if="scope.row.type=='dir'">
              ...
              <el-button type="text" size="small"
                         @click="openDelete(scope.row.data.folderId,scope.row.type)">删除
              </el-button>
              <el-button type="text" size="small"
...
export default {
  ...
  methods: {
    ...
    //上传文件
    async beforUploadFile(file) {

      const reader = new FileReader();
      reader.readAsArrayBuffer(file);
      const md5String = await new Promise((resolve) => {
        reader.onload = (e) => {
          const arrayBuffer = e.target.result;
          const blob = new Blob([new Uint8Array(arrayBuffer)]);
          resolve(this.getMD5(blob));
        };
      });
      console.log(md5String)
      let taskId;
      await uploadFile(this.$route.params.path, file, md5String).then(res => {
        if (res.code == 20000) {
          this.$message({
            type: 'info',
            message: '上传文件中!'
      ...
      console.log("开始查看是否上传成功")
      let isretry = true;
      while (isretry) {
        await checkUpload(taskId).then(res => {
    ...
    async getFileList() {
      ...
      await getFiles(this.$route.params.path).then(res => {
        let files = res.data;
        if (res.code = 20000) {
          for (var i = 0; i < files.length; i++) {
            let node = {
    ...
    async getFolderList() {
      ...
      await getUserPaths(this.$route.params.path).then(res => {
        if (res.code == 20000) {
