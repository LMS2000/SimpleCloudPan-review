        ...
        <input
          ...
          @keyup.enter="
            rightClickFolderItem.id
              ...
              : ''
          "
          v-if="isFolderRenameInputShow && rightClickFolderItem.id == item.folderId"
        />
        <div class="name" v-else>
          {{
            item.folderName == null
              ? "NoNameFolder"
              : item.folderName.substr(0, item.folderName.length - 1)
          }}
        </div>
      </div>
      ...
      >
        <div class="imgContainer">
          ...
          <img
            :src="item.fileUrl"
            alt=""
            v-else-if="item.fileType == 'image'|| item.fileType =='png'"
            :draggable="false"
          />

          <!-- 视频文件 -->
          <div class="videoContainer" v-else-if="item.fileType == 'video'">
            <video :src="item.fileUrl" class="videoItem" preload="meta"></video>
       ...
       <!--   <img
        ...
        <input
          ...
          "
          v-if="isRenameShow && rightClickItem.id == item.fileId"
        />
        <div class="name" v-else>
    ...
    <div class="tableTypeList" v-else>
      ...
      >
        ...
        <div class="tableName" v-else>
          {{ item.fileName == null ? "NoNameFile" : item.fileName + "." + item.fileType }}
        </div>
...
import {deleteDir} from '../../api/folder.js'
import {deleteFiles,moveFiles,renameFile} from '../../api/file.js'
...
import {createDir} from '../../api/folder.js'
export default {
  components: {
    Attribute,
    ImagePlayer,
  ...
  methods: {
    ...
    async renameDone(item, index, type) {
      ...
      if (!type) {
        ...
        if (this.rightClickItem.name != this.renameInput.trim()) {
          ...
          //   "post"
          // );
					let res = await renameFile(item.fileId,this.renameInput.trim())
          console.log(res);
          if (res.code==20000) {
      ...
      this.$nextTick(async () => {
        ...
        if (input) {
          if (this.createdName == "") {
            ...
            // }
						let folder={
							path:this.createdName,
							parentPathId:this.currentFolderId
						}
						console.log(folder)
						createDir(folder).then(res=>{
							if(res.code==20000){
								this.$emit("getFolderList");
								this.$store.commit("updateIsGetingFolder", true);
							}else{
      ...
      if (res.code==20000) {
        if (type == "current") {
          let index = this.listData.findIndex(
            (item) => item.fileId == this.rightClickItem.id
          );
          ...
          userInfo.useQuota -= item.size;
          this.$store.commit("updateUserInfo", userInfo);
        } else {
          let deleteSize = 0;
          this.selectFiles.forEach((item) => {
            let index = this.listData.findIndex((i) => i.id == item.fileId);
            this.listData.splice(index, 1);
    ...
    openCurrentFolder(item) {
      ...
      let currentFolder = (
        "/" + this.findPathByLeafId(item.fileName, [this.folderList]).join("")
      ).slice(
        0,
        this.findPathByLeafId(item.fileName, [this.folderList]).join("").length
      );
      // let currentFolder =
      ...
      if (arr.length > 1) {
        ...
        for (var i = 1; i < arr.length; i++) {
          let index = this.currentChildrenFolder.findIndex(
            (item) => item.fileName.substr(0, item.fileName.length - 1) == arr[i]
          );
    ...
    async confirmMove(path, type, list) {
      ...
      //   "json"
      // );
			let res = await moveFiles(arr,path)
      console.log(res);
      if (res.code==20000) {
    ...
    async deleteCurrentFolder(item) {
      ...
      //   "json"
      // );
			let res = await deleteDir(item.id)
      // console.log(res);
      if (res.code==20000) {
        this.$emit("getFolderList");
      }
